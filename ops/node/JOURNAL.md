...
- Both copies of `drift.cmd` and both copies of the skill have equal SHA-256 hashes. `drift` resolves on the PATH to `C:\Users\joshi\.local\bin\drift.cmd`. Claude Code lists `alienware-node` as an available skill.
