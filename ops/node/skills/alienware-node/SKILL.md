...
- **HARD RULE, above every other ruling** (Joshua, 2026-09-19, confirmed in his own words in this lane). Never violate Anthropic's terms of service, or any provider's or licence's terms, and never do anything that could risk his Claude subscription. If a guardrail, permission check, policy, licence or sign-in wall blocks an action, leave it blocked. Do not look for another route to the same blocked result and do not change modes or settings to get past it. Say in one line what was blocked and carry on with the rest. Write this rule into every Hermes card.
...
3. `git -C C:\DREAM\dream-online status --short` and `git -C C:\DREAM\dream-online fetch` then compare with `origin/main`. The whole folder is expected clean: `git status --short` prints nothing. Anything untracked there is new drift; find out what it is before doing anything else.
...
- This box has no C++ compiler (checked 2026-09-20). Claude Code's permission classifier refused the Visual Studio install, the merge into the hermes `master`, and the move of the live hermes checkout; under the hard rule those stay blocked for Claude and wait for Joshua. `C:\DREAM\recon\QUEUE.md` says where each one stands.
